...
def ConfirmNoReviewResponse(bot, message, User):
    ...
    if check_for_expected_input(message, positives):
        ...
        next_message = "Ok, what is your review?"
        next_stage = "GiveReview"
    stageContext = db.getStage(next_context) if no_review else "filmReview"
    next_context = next_context if type(next_context) is int else contexts[next_context]
    bot.send_message(User.id, next_message)
    setUserContextAndStage(User.id, next_context, stages[stageContext][next_stage])
...
def ScoreFilmResponse(bot, message, User):
    ...
    stageContext = db.getStage(next_context) if ratingGiven else "filmReview"
    next_context = next_context if type(next_context) is int else contexts[next_context]
    bot.send_message(User.id, next_message)
    setUserContextAndStage(User.id, next_context, stages[stageContext][next_stage])
